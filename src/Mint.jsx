...
const nftContract = getContract({
  client: client,
  chain: polygon,
  address: "0xd8dBdFdFb43b3Ca997F33F4A617B89dE36eE76E0"
});
...
function Mint() {
  ...
  const pricePerNFT = BigInt("200000000000000");

  const loadMinted = async () => {
    try {
      // Leggiamo direttamente il contatore totalSupply dal nuovo contratto
      ...
      // Aggiorniamo lo stato con il numero reale di NFT venduti
      setMinted(Number(total)); 

    } catch (err) {
      console.error("Errore lettura totalSupply:", err);
      setMinted(0); 
    }
  };
  ...
  const handleMint = async () => {
    ...
    try {
      ...
      const isWhitelisted = await readContract({
        contract: nftContract,
        method: "function whitelistedAddresses(address) view returns (bool)",
      ...
      // --- CONTROLLO LIMITE 5 PER WALLET ---
      const userMinted = await readContract({
        contract: nftContract,
        method: "function addressMintedBalance(address) view returns (uint256)",
        params: [account.address]
      });

      if (Number(userMinted) + Number(quantity) > 5) {
        alert ( "Limit reached! Do you already have 5 NFTs or are you exceeding the limit" );
        setLoading(false);
        return;
      }
    ...
    <div className="mint-global-wrapper" style={{
      ...
      <div className="mint-main-box" style={{
        ...
        <div style={{
          ...
          PRICE  NFT: 0.0002 WETH
          <br/>
          TOTAL: {Number(0.0002 * quantity).toFixed(4)} WETH
        </div>
